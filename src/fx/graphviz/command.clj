...
(defn map-edges
  "helper function for mapping edges"
  {:added "3.0"}
  ([m]
   (mapcat (fn [[k vs]]
             (for [v vs] [k v]))
...
(defn graph->dot
  "converts a graph to dot notation"
  {:added "3.0"}
  ([nodes edges options]
   (let [directed? (:directed? options false)
     ...
     (apply str
            (cond current-cluster
                  (str "subgraph cluster_"
                       (if (empty? (cluster->id current-cluster))
                         "none"
                         (cluster->id current-cluster)))
           ...
           ;; format subclusters
            (let [clusters (->> clusters
                                (filter (comp #{current-cluster} cluster->parent))
                                (remove nil?))]
