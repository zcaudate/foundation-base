...
^{:refer lib.redis.event/listener:create :added "3.0"
  :use [|client|]}
(fact "creates a listener" ^:hidden
  ...
  (cc/req |client| ["PUBLISH" "foo" (str {:a 1 :b 2})])
  => 1
...
^{:refer lib.redis.event/listener:teardown :added "3.0"
  :use [|client|]}
(fact "tears down the listener" ^:hidden
...
^{:refer lib.redis.event/listener:add :added "3.0"
  :use [|client|]}
(fact "adds a listener to the redis client" ^:hidden
  ...
  (cc/req |client| ["PUBLISH" "foo" "HELLO"])
  => 1
...
^{:refer lib.redis.event/listener:remove :added "3.0"
  :use [|client|]}
(fact "removes a listener from the client" ^:hidden
...
^{:refer lib.redis.event/listener:all :added "3.0"
  :use [|client|]}
(fact "lists all listeners" ^:hidden
...
^{:refer lib.redis.event/listener:count :added "3.0"
  :use [|client|]}
(fact "counts all listeners" ^:hidden
...
(fact "wrapper for the subscribe delivery function" ^:hidden
...
^{:refer lib.redis.event/subscribe :added "3.0"
  :use [|client|]}
(fact "subscribes to a channel on the cache" ^:hidden
...
^{:refer lib.redis.event/unsubscribe :added "3.0"
  :use [|client|]}
(fact "unsubscribes from a channel" ^:hidden
...
(fact "wrapper for the psubscribe delivery function" ^:hidden
...
^{:refer lib.redis.event/psubscribe :added "3.0"
  :use [|client|]}
(fact "subscribes to a pattern on the cache" ^:hidden
...
^{:refer lib.redis.event/punsubscribe :added "3.0"
  :use [|client|]}
(fact "unsubscribes from the pattern" ^:hidden
...
(fact "creates a string from a set of enums" ^:hidden

  (events-string #{:string :hash :generic})
  => "h$g")

^{:refer lib.redis.event/events-parse :added "3.0"}
(fact "creates a set of enums from a string" ^:hidden
...
^{:refer lib.redis.event/config:get :added "3.0"
  :use [|client|]}
(fact "gets the config for notifications" ^:hidden
...
(fact "produces the notify args" ^:hidden
...
^{:refer lib.redis.event/notify:wrap :added "3.0"
  :use [|client|]}
(fact "wrapper for the notify delivery function" ^:hidden
...
^{:refer lib.redis.event/notify :added "3.0"
  :use [|client|]}
(fact "notifications for a given client" ^:hidden
...
^{:refer lib.redis.event/unnotify :added "3.0"
  :use [|client|]}
(fact "removes notifications for a given client" ^:hidden
...
(comment
  (./import)
  (cc/req |client| ["PUBLISH" "bar" (.getBytes "HELLO")])
